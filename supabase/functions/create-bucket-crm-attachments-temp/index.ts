...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for crm-attachments" ON storage.objects FOR SELECT USING (bucket_id = 'crm-attachments');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for crm-attachments" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'crm-attachments');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for crm-attachments" ON storage.objects FOR UPDATE USING (bucket_id = 'crm-attachments');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for crm-attachments" ON storage.objects FOR DELETE USING (bucket_id = 'crm-attachments');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'crm-attachments',
        public: true,
        allowed_mime_types: ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/*"],
